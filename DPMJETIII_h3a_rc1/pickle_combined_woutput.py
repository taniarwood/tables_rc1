...
def redoFile(infile_name, outfile_name):
 ...
 #make the weights
    ...
    e_k =   data['weight_DMP_h3a_rc1_flat_e_k'][nubool*ebool]
    print e_k
    ...
    ae_k = data['weight_DMP_h3a_rc1_flat_e_k'][~nubool*ebool]
    print ae_k
    ...
    e_p = data['weight_DMP_h3a_rc1_flat_e_p'][nubool*ebool] 
    print e_p
    ...
    epa = data['weight_DMP_h3a_rc1_flat_e_p'][~nubool*ebool]
...
#NOW PUT IN KEYS WITH THE DMPJETIII h3a Spectrum as is, not flat
 ...
 #make the weights
    ...
    combined = np.sum(data['weight_DMP_h3a_rc1_e_k'] + data['weight_DMP_h3a_rc1_e_p'])
    print 'Stuff', combined, combined.mean(), combined.max(), combined.min()
    print '                                                            '

    print 'Old weights NuMu','np.sum', 'mean, max, min' 
    print 'Stuff', np.sum(data['weight_mu']), data['weight_mu'].mean(), data['weight_mu'].max(), data['weight_mu'].min()
    print 'New weights NuMu', 
#    combinedmu = np.sum(data['weight_DMP_h3a_rc1_flat_mu_k']) + np.sum( data['weight_DMP_h3a_rc1_flat_mu_p'])
    combinedmu = np.sum(data['weight_DMP_h3a_rc1_mu_k']) + np.sum( data['weight_DMP_h3a_rc1_mu_p'])
    print 'Stuff', combinedmu, combinedmu.mean(), combinedmu.max(), combinedmu.min()
...
#    out_dir = '/gs/project/ngw-282-ac/trwood/MSU_contain_removed/flat_tania3_oct11/'
    ...
    out_dir  ='/gs/project/ngw-282-ac/trwood/jasper_home/MSU_contain_removed/flat_tania3_DPM_interm_BAKOct12017_jaspert_BKBK/def2_copy'
